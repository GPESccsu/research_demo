...
import { saveConfig, loadConfig, getPapers, savePapers, addPaper, deletePaper, getLogs, saveLogs, addLog, getChecklist, saveChecklist, getChatHistory, saveChatMessage, clearChatHistory, getSynonymGroups, saveSynonymGroup, deleteSynonymGroup, getClips, addClip, updateClip, deleteClip, getDrafts, saveDraft, getUIState, saveUIState, initializeDatabase, exportDatabaseSnapshot, importDatabaseSnapshot, getSearchQueries, addSearchQuery, updateSearchQuery, deleteSearchQuery } from "./db.js";
...
function ReadingPage(){
  const CATEGORIES = ['前言素材', '实验方法', '图片素材', '结果讨论'];
  const [pdfName, setPdfName] = useState('');
  const [clipText, setClipText] = useState('');
  const [msg, setMsg] = useState('');
  const [clips, setClips] = useState([]);
  const [currentCategory, setCurrentCategory] = useState('前言素材');
  const [showAddForm, setShowAddForm] = useState(false);
  const [selectedClip, setSelectedClip] = useState(null);
  const [editingClip, setEditingClip] = useState(null);
  const [newClip, setNewClip] = useState({ title: '', content: '', source: '', tags: '', category: '前言素材' });
  const [quickClipCategory, setQuickClipCategory] = useState('前言素材');
  ...
  const addManualClip = async () => {
    ...
    setClipText('');
    setCurrentCategory(quickClipCategory);
    setMsg('素材已加入素材库');
    setTimeout(() => setMsg(''), 3000);
  };
  ...
  const handleAddClip = async () => {
    ...
    const id = await addClip(clip);
    setClips(prev => [{ ...clip, id }, ...prev]);
    setNewClip({ title: '', content: '', source: '', tags: '', category: currentCategory });
  ...
  // 获取当前分类的素材
  const currentClips = clips.filter(c => c.category === currentCategory);
  
  // 获取各分类的素材数量
  const getCategoryCount = (category) => clips.filter(c => c.category === category).length;
  ...
  return(
    <div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              ...
              <select 
                ...
                value={quickClipCategory} 
                onChange={e=>setQuickClipCategory(e.target.value)}
      ...
      {/* 素材库展示区域 */}
      <div style={{display:'grid',gridTemplateColumns:'200px 1fr 360px',gap:16,minHeight:500}}>
        {/* 左侧：分类列表 */}
        <div className="panel fade-in">
          <div className="panel-header"><Icons.Folder/>分类</div>
          <div className="panel-body">
            <div style={{display:'flex',flexDirection:'column',gap:4}}>
              {CATEGORIES.map(cat=>(
                <div 
                  key={cat}
                  className={`group-item ${currentCategory===cat?'active':''}`}
                  onClick={()=>setCurrentCategory(cat)}
                  style={{cursor:'pointer'}}
                >
                  <span style={{fontSize:16}}>
                    {cat==='前言素材'?'📝':cat==='实验方法'?'🧪':cat==='图片素材'?'🖼️':'📊'}
                  </span>
                  <span style={{flex:1}}>{cat}</span>
                  <span className="group-count">{getCategoryCount(cat)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="panel fade-in delay-1">
          <div className="panel-header">
            <Icons.Database/>{currentCategory}
            <span style={{marginLeft:'auto',fontSize:12,color:'var(--text-muted)',fontFamily:'var(--font-mono)'}}>
              {currentClips.length} 条
            </span>
            <button className="btn btn-primary btn-sm" onClick={()=>{setNewClip(p=>({...p,category:currentCategory}));setShowAddForm(true);}} style={{marginLeft:8}}>
              <Icons.Plus/>添加
            </button>
          </div>
          <div className="panel-body" style={{maxHeight:600,overflowY:'auto'}}>
              ...
              <div style={{marginBottom:16,padding:16,background:'var(--bg-elevated)',border:'1px solid var(--accent-amber)',borderRadius:'var(--radius-md)'}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  <select 
                    ...
                    onChange={e=>setNewClip(p=>({...p,category:e.target.value}))}
                  >
                    {CATEGORIES.map(cat=><option key={cat} value={cat}>{cat}</option>)}
                  </select>
                  <input 
                  ...
                  <div style={{display:'flex',gap:8}}>
                    ...
                    <button className="btn btn-secondary btn-sm" onClick={()=>{setShowAddForm(false);setNewClip({title:'',content:'',source:'',tags:'',category:currentCategory});}}>取消</button>
                  </div>
                </div>
              </div>
            )}
              ...
              <div style={{marginBottom:16,padding:16,background:'var(--bg-elevated)',border:'1px solid var(--accent-blue)',borderRadius:'var(--radius-md)'}}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  <select 
                    ...
                    onChange={e=>setEditingClip(p=>({...p,category:e.target.value}))}
                  >
                    {CATEGORIES.map(cat=><option key={cat} value={cat}>{cat}</option>)}
                  </select>
                  <input 
            ...
            {currentClips.length === 0 && !showAddForm && (
              <div style={{textAlign:'center',padding:40,color:'var(--text-muted)'}}>
                <div style={{fontSize:48,marginBottom:12,opacity:.3}}>📚</div>
                <div style={{fontSize:14}}>暂无素材，使用上方快速摘录或点击"添加"按钮</div>
              </div>
            )}
                ...
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:11,color:'var(--text-muted)'}}>
                      {selectedClip.category} · {selectedClip.type==='auto'?'自动提取':'手动添加'}
                    </div>
                  </div>
                  <button 
                    className="btn btn-secondary btn-sm" 
